...
def generate_db_status_report(output_file=None):
    ...
    try:
        ...
        # Collect all necessary database information
        db_info = {
        ...
        try:
            listener_checker = ListenerChecker(oracle_home)
            listener_info = listener_checker.check_all_listeners()
        except Exception as e:
            print(f"Warning: Could not check listener status: {e}", file=sys.stderr)
            listener_info = None

        # Generate HTML report
        report_html = HTMLReportGenerator.generate_db_status_report(db_info, listener_info)
